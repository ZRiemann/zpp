...
const char *z::str_err(z::err_t err) {
  ...
  static const char *errs[] = {
      "[0]err-ok",           "[1]err-fail",
      "[2]err-not-support",  "[3]err-memory-insufficient",
      "[4]err-out-of-range", "[5]err-not-exit",
      "[6]err-exist",        "[7]err-param-invalid",
      "[8]err-again",        "[9]err-timeout",
      "[10]err-end"};
  ...
  if ((z::ERR_MASK & err) == z::ERR_MASK) {
    ...
    if (code >= NERR_BASE && code <= MAX_NERR) {
      hModule =
          LoadLibraryEx(TEXT("netmsg.dll"), NULL, LOAD_LIBRARY_AS_DATAFILE);
      if (hModule != NULL)
        dwFormatFlags |= FORMAT_MESSAGE_FROM_HMODULE;
    }

    if (dwBufferLength =
            FormatMessageA(dwFormatFlags, hModule, err,
