...
bool is_tcp_url(std::string_view url) { return starts_with(url, tcp_prefix); }

bool is_ipc_url(std::string_view url) { return starts_with(url, ipc_prefix); }
...
z::err_t remove_stale_ipc_socket(std::string_view url, std::string *error) {
  ...
  if (ec) {
    if (error != nullptr) {
      *error =
          "failed to remove stale ipc socket " + path + ": " + ec.message();
    }
    return z::ERR_FAIL;
  }
  return z::ERR_OK;
...
z::err_t validate_endpoint(const endpoint &target) {
  const auto parsed_transport = parse_transport(target.url);
  if (target.url.empty() || parsed_transport == transport::unknown ||
      parsed_transport != target.transport ||
      (target.role != endpoint_role::listen &&
       target.role != endpoint_role::dial)) {
    nng2errf(cmp_endpoint, act_validate,
             "invalid endpoint role={} transport={} url={}",
             endpoint_role_name(target.role), transport_name(target.transport),
             target.url);
    return z::ERR_FAIL;
  }
  return z::ERR_OK;
...
z::err_t validate_endpoints(const std::vector<endpoint> &endpoints) {
  if (endpoints.empty()) {
    nng2errf(cmp_endpoint, act_validate, "endpoint list is empty");
    return z::ERR_FAIL;
  }
  for (const auto &target : endpoints) {
    if (validate_endpoint(target) != z::ERR_OK) {
      return z::ERR_FAIL;
    }
  }
  return z::ERR_OK;
