...
constexpr std::size_t stats_size() { return sizeof(ServiceStats); }

} // namespace

inspector_server::~inspector_server() { fini(); }
...
bool inspector_server::init(std::string_view name) {
  ...
  auto try_create = [&]() {
    auto *h =
        new shm_handle{bip::shared_memory_object(
...
inspector_client::~inspector_client() { fini(); }
...
bool inspector_client::init(std::string_view name) {
  ...
  try {
    auto *h = new shm_handle{
        bip::shared_memory_object(bip::open_only, std::string{name}.c_str(),
                                  bip::read_only),
        bip::mapped_region{}};
