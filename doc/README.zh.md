...
zpp 作为独立 ZetaX 项目构建。它只在自己的 `builder/` 目录下维护 zpp 特有构建逻辑；共享 Conan 依赖版本和 package config 由 `zeta_forge deps` 提供。
当前项目版本记录在仓库根目录 `VERSION`，发布历史见 `release_history.org`。
`VERSION` 是唯一版本源；CMake 会读取它并派生 `project(zpp VERSION ...)`。
...
zpp 不运行 Conan；如果找不到 `spdlog`、`fmt` 或 `GTest` package config，请先运行 `zeta_forge/zbuild.py deps --install` 安装共享依赖环境。
zpp 的 CMake 配置阶段不再对 `ZETA_CMAKE_UTIL_DIR` 与 `ZETA_DEPS_CMAKE_DIR` 做 fallback 自动推导。请先完成 zeta_forge 环境准备，再通过 `./zbuild.py` 构建 zpp。
...
旧的 `cbuild` helper 不再作为 zpp 推荐入口。它会执行独立 CMake 配置，可能拿不到 zeta_forge 安装的共享依赖 package config。
如果必须直接使用 CMake，请显式传入 `-DZETA_CMAKE_UTIL_DIR=...` 与 `-DZETA_DEPS_CMAKE_DIR=...`。
