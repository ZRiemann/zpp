...
public:
  task_benchmark(size_t tasks) : _tasks(tasks), _elapsed_ms(0), _done(false) {
    _hits = new int[_tasks];
    for (size_t i = 0; i < _tasks; ++i) {
      _hits[i] = 0;
    }
  }
  ~task_benchmark() { delete[] _hits; }
  ...
  void print() {
    ...
    spd_inf(
        "\n\tcheck fails {}\n\ttotal tasks: {}\n\telapsed ms: {}\n\tq/s: {}\n",
        fails, CTS(_tasks), CTS(_elapsed_ms),
        CTS((_tasks / _elapsed_ms) * 1000));
  }
...
public: // task pool
  ...
  inline bool free() { return pool.push(this); }
};
