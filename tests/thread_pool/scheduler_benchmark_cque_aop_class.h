...
public:
  ...
  inline bool push() { return pool.push(this); }
};
/**
 * @brief aop 调度器
 */
  ...
  void schedule() {
    ...
    do {
      ...
      case 0:
        ...
        break;
      case MAX_COUNT - 1:
        spd_inf("max_count[{}] reached. elapsed {} ms, avg {} q/s", MAX_COUNT,
                _stopwatch.elapsed_ms(),
                CTS((MAX_COUNT / _stopwatch.elapsed_ms()) * 1000));
        _done = true;
        break;
      default:
        break;
...
public:
  ...
  timer<> _stopwatch;
  int _max_index{
      0};     // 排查 t->push() 导致的无法达到退出问题, _done = true 不可达！
