...
public:
  task_benchmark_engine_fifo(stopwatch_tsc &sw, size_t qid, size_t id,
                             size_t end_task)
  ...
  void operate() override {
    if (_id == 0) {
      ...
      spd_inf(
          "que[{}] count[{}] time[{}][now:{}] done. Q/S:{}", _qid,
          CTS(_end_task + 1), CTS(ns),
          CTS(std::chrono::duration_cast<std::chrono::nanoseconds>(
 ...
 * @note 永远都不结束任务
 */
template <typename Scheduler> class task_infinite : public aop {
public:
  task_infinite(Scheduler &scheduler, size_t id)
      : _scheduler(scheduler), _id{id}, _count{0} {}
  ~task_infinite() = default;

  void operate() override {
    if (!(_count & 0x3ffff)) {
      spd_inf("thr[{}] task[{}] repeats[{}]", tid::id(), fmt::ptr(this),
              _count);
    }
    ++_count;
    ...
    while (!ret) {
      z::sleep(1);
      spd_war("thr[{}] task[{}] dispatch failed, try again...", tid::id(),
              fmt::ptr(this));
