...
protected:
  void SetUp() override { z::tsc_init(); }
...
TEST_F(TimestampCounterTest, RelaxedCounterIsMonotonic) {
  ...
  EXPECT_TRUE(observed_increase)
      << "TSC should progress across successive relaxed reads";
}
...
TEST_F(TimestampCounterTest, SerializedCounterIsMonotonic) {
  ...
  EXPECT_TRUE(observed_increase)
      << "TSC should progress across successive serialized reads";
}
...
TEST_F(TimestampCounterTest, FrequencyOverrideControlsConversion) {
  ...
  const z::tsc_t cycles = static_cast<z::tsc_t>(
      z::get_tsc_frequency()); // ~1 second worth of cycles
  ...
  const auto diff =
      static_cast<int64_t>(nanos) - static_cast<int64_t>(expected);
  EXPECT_GE(diff, -1);
  EXPECT_LE(diff, 1);
}
