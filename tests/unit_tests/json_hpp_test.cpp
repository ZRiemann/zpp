...
TEST(JsonHppTest, ParseReadAndSerialize) {
  z::json conf;
  ASSERT_EQ(z::ERR_OK,
            conf.load_string("{\"server\":{\"spd\":{\"max_size\":32}},"
                             "\"fronts\":[\"tcp://a\",\"tcp://b\"]}"));
...
TEST(JsonHppTest, BuildObjectArrayAndNestedValues) {
  ...
  ASSERT_EQ(
      z::ERR_OK,
      doc.set("obj", obj)); // obj is still valid after being copied into arr
  ASSERT_EQ(z::ERR_OK,
            arr.push(std::move(obj))); // Move obj into arr, obj becomes invalid
