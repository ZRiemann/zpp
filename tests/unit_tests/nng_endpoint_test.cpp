...
TEST(NngEndpoint, ParsesTransportFromUrls) {
    EXPECT_EQ(z::nng::parse_transport("tcp://127.0.0.1:58200"), z::nng::transport::tcp);
    EXPECT_EQ(z::nng::parse_transport("ipc:///tmp/a.sock"), z::nng::transport::ipc);
    EXPECT_EQ(z::nng::parse_transport("inproc://worker"), z::nng::transport::inproc);
    EXPECT_EQ(z::nng::parse_transport("udp://127.0.0.1:58200"), z::nng::transport::unknown);
}
...
TEST(NngEndpoint, ValidatesTcpPortsAndFormatsUrls) {
    ...
    EXPECT_EQ(z::nng::make_tcp_url("203.0.113.10", 58201), "tcp://203.0.113.10:58201");
}

TEST(NngEndpoint, ReplacesUrlTokens) {
    EXPECT_EQ(z::nng::replace_url_token("ipc:///tmp/zeta.out.sock", ".out.", ".in."),
              "ipc:///tmp/zeta.in.sock");
    EXPECT_EQ(z::nng::replace_url_token("ipc:///tmp/zeta.out.sock", ".out.", ".rep."),
...
TEST(NngEndpoint, ExtractsIpcPathAndRemovesStaleSockets) {
    ...
    std::string error;
    EXPECT_EQ(z::nng::remove_stale_ipc_socket("tcp://127.0.0.1:58200", &error), z::ERR_OK);
    ...
    EXPECT_EQ(z::nng::remove_stale_ipc_socket("ipc://" + path, &error), z::ERR_OK);
