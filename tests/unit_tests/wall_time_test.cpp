...
TEST(WallTimeTest, EpochMillisecondsTracksSystemClock) {
  const auto before = std::chrono::duration_cast<std::chrono::milliseconds>(
                          z::wall_time::clock_type::now().time_since_epoch())
                          .count();
...
TEST(WallTimeTest, FormatsTimePointWithSixMicrosecondDigits) {
  ...
  EXPECT_TRUE(std::regex_match(
      out, std::regex(R"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.\d{6})")));
}
...
TEST(WallTimeTest, EpochNanosecondsIsAtLeastEpochMicroseconds) {
  const auto epoch_us = z::wall_time::epoch_us();
  const auto epoch_ns = z::wall_time::epoch_ns();

  EXPECT_GE(epoch_ns, epoch_us * 1000);
}
