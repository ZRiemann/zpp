...
protected:
  ...
  void TearDown() override { mon_guard::print_statistic(); }
  ...
  也可称为：concurrent gather, parallel aggregation, concurrent result
  aggregation 相关 API/术语： merge（合并/归并流） collectAll（批量收集/聚合）
...
TEST_F(FollyCoroTest, ZFoCoBasicTest) {
  ...
  // 手动执行并等待协程完成（将所有权移动给 blockingWait）
  int v =
      z::fo::co_blocking_wait(std::move(z::fo::co_cpu(sample_folly_task())));
  EXPECT_EQ(v, 42);
}
...
TEST_F(FollyCoroTest, IoWorkTest) {
  // 等待阻塞IO协程完成
  auto ioTaskWithExec = folly::coro::co_withExecutor(
      folly::getGlobalIOExecutor(), blocking_io_work());
  // 如果到达这里，说明协程已成功完成
...
TEST_F(FollyCoroTest, CPUWorkTest) {
  // auto cpuTaskWithExec =
  // folly::coro::co_withExecutor(folly::getGlobalCPUExecutorWeakRef(),
  // cpu_work(1000000));
  auto cpuTaskWithExec = folly::coro::co_withExecutor(
      folly::getGlobalCPUExecutor(), cpu_work(1000000));
