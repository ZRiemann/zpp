...
{
public:
    fake_server(int argc, char** argv)
        :_argc(argc)
        ,_argv(argv)
    {
    ...
    ~fake_server()
    {
        state().events.emplace_back("destruct");
    }
    ...
    void loop()
    {
        state().events.emplace_back("loop");
    }
...
TEST(HpxServerLifecycleTest, RunFailureSkipsLoopAndCleanup)
{
    ...
    EXPECT_EQ(z::ERR_TIMEOUT, z::zhpx::run_server_lifecycle<fake_server>(1, argv));

    const std::vector<std::string> expected{
        "construct", "configure", "run", "destruct"};
    EXPECT_EQ(expected, events());
}
...
TEST(HpxServerLifecycleTest, StopFailureStillWaitsAndReturnsFirstCleanupError)
{
    ...
    const std::vector<std::string> expected{
        "construct", "configure", "run", "loop", "stop", "wait_stop",
        "destruct"};
    EXPECT_EQ(expected, events());
}
