  ...
  std::jthread consumer([&que](std::stop_token token) {
    ...
    while (run) {
      if (!que.pop(val)) {
        ...
        spd_war("front_ptr:{}, back_ptr:{}, front_end_ptr:{}, back_end_ptr:{}, "
                "back_local:{}, back_cached:{}",
                fmt::ptr(que.front_ptr_),
                fmt::ptr(que.back_ptr_.load(std::memory_order_acquire)),
                fmt::ptr(que.front_end_ptr_), fmt::ptr(que.back_end_ptr_),
                fmt::ptr(que.back_local_), fmt::ptr(que.back_cached_));
#endif
        ++count_fails;
        continue;
      }
      if (cnt != val) {
        spd_err("not match cnt[{}] val[{}] diff[{}]", cnt, val,
                cnt > val ? cnt - val : val - cnt);
        spd_err("front_ptr:{}, back_ptr:{}, front_end_ptr:{}, back_end_ptr:{}, "
                "back_local:{}, back_cached:{}",
                fmt::ptr(que.front_ptr_),
                fmt::ptr(que.back_ptr_.load(std::memory_order_acquire)),
                fmt::ptr(que.front_end_ptr_), fmt::ptr(que.back_end_ptr_),
      ...
      if (!(val & PRINT_MASK)) {
        ...
        spd_inf("consum {} use {} us. {} q/s pop_fails[{}]", CTS(val), CTS(us),
                CTS((val / us) * 1000000), CTS(count_fails));
      }
      switch (val) {
      case 0:
      ...
      case END_COUNT: {
        auto us = stop_watch.elapsed_us() + 1;
        spd_inf("consum done. item[{}] use {} us. {} q/s", CTS(MAX_COUNT),
                CTS(us), CTS((MAX_COUNT / us) * 1000000), CTS(count_fails));
        run = false;
      } break;
      default:
        break;
...
void stl_svr::pressure_ring_compare() {
  ...
  for (size_t publish_batch : batches) {
    spd_inf("benchmark RING(cap:{}, publish_batch:{}, count:{})",
            CTS(RING_CAPACITY), CTS(publish_batch), CTS(RING_MAX_COUNT));
    ...
    std::jthread consumer([&que](std::stop_token token) {
      ...
      while (run) {
        ...
        if (cnt != val) {
          spd_err("[ring] not match cnt[{}] val[{}] diff[{}]", cnt, val,
                  cnt > val ? cnt - val : val - cnt);
          run = false;
        }
        ...
        if (!(val & PRINT_MASK)) {
          auto us = stop_watch.elapsed_us() + 1;
          spd_inf("[ring] consum {} use {} us. {} q/s pop_fails[{}]", CTS(val),
                  CTS(us), CTS((val / us) * 1000000), CTS(pop_fails));
        }
        ...
        case RING_END_COUNT: {
          auto us = stop_watch.elapsed_us() + 1;
          spd_inf(
              "[ring] consum done. item[{}] use {} us. {} q/s pop_fails[{}]",
              CTS(RING_MAX_COUNT), CTS(us),
              CTS((RING_MAX_COUNT / us) * 1000000), CTS(pop_fails));
          run = false;
        } break;
        default:
          break;
...
stl_svr::stl_svr(int argc, char **argv) : server(argc, argv) {}
