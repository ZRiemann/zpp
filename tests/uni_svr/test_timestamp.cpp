...
void z::test_timestamp_counter() {
  ...
  spd_inf("TSC frequency detected: {} Hz {} ns/hz", CTS(get_tsc_frequency()),
          CTS(get_tsc_ns_per()));
  ...
  spd_inf("TSC serialized value read: {}, {}ns", CTS(tsc),
          CTS(ns_serialized_end));

  spd_inf("TSC relaxed elapsed: {} ns", CTS(ns_relaxed_end - ns_relaxed));
  spd_inf("TSC serialized elapsed: {} ns",
          CTS(ns_serialized_end - ns_serialized));
