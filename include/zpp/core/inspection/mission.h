...
inline constexpr uint64_t pack_mission_info(const mission_info &mi) noexcept {
  return (static_cast<uint64_t>(mi.mid) << 32) |
         ...
         (static_cast<uint64_t>(mi.steps) << 8) | static_cast<uint64_t>(mi.tid);
}
...
[[nodiscard]] inline mission_info
unpack_mission_info(const uint64_t info) noexcept {
  return mission_info{static_cast<uint8_t>(info & 0xFFu),
...
static_assert(std::is_standard_layout_v<mission>,
              "mission must be standard-layout for offsetof to be valid");
static_assert(alignof(mission) >= alignof(duration_t),
              "mission must be at least duration_t aligned");
static_assert(offsetof(mission, elapsed) % alignof(duration_t) == 0,
              "elapsed[] must start at duration_t alignment");
