...
public:
  ...
  template <typename Awaitable> inline void add_cpu(Awaitable &&a) {
    scope_.add(
        folly::coro::co_withExecutor(keep_cpu_, std::forward<Awaitable>(a)));
  }

  // Same as add_cpu but the task is tracked by the cancellable scope.
  template <typename Awaitable> inline void add_cpu_c(Awaitable &&a) {
    cancellable_scope_.add(
        folly::coro::co_withExecutor(keep_cpu_, std::forward<Awaitable>(a)));
  }

  template <typename Awaitable>
  inline folly::coro::Task<void> co_cpu(Awaitable &&a) {
    co_await folly::coro::co_withExecutor(keep_cpu_,
  ...
  inline folly::coro::Task<void> co_io(Awaitable &&a) {
    co_await folly::coro::co_withExecutor(keep_io_, std::forward<Awaitable>(a));
    co_return;
  }
  ...
  template <typename Awaitable>
  inline folly::coro::Task<void> co_exe(Awaitable &&a,
                                        folly::Executor::KeepAlive<> keep) {
    co_await folly::coro::co_withExecutor(keep, std::forward<Awaitable>(a));
    co_return;
  }
