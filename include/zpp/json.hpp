...
#define RAPIDJSON_NOMEMBERITERATORCLASS // disable struct std::iterator is
                                        // deprecated
...
template <class V = rapidjson::Document> class json {
public:
  using value_type = V;
  ...
  static_assert(is_document || is_value || is_view,
                "z::json only supports rapidjson::Document, rapidjson::Value "
                "and rapidjson::Value*.");
  ...
  public:
    ...
    iterator() : ptr_(nullptr), alloc_(nullptr) {}

    iterator(rapidjson::Value *ptr, allocator_type *alloc)
        : ptr_(ptr), alloc_(alloc) {}

    reference operator*() const { return value_type(ptr_, alloc_); }
  ...
  json()
    requires(is_document)
      : v(), alloc_(&v.GetAllocator()) {}
  ...
  json()
    requires(is_view)
      : v(nullptr), alloc_(nullptr) {}

  template <class OwnerV>
  json(json<OwnerV> &owner)
    requires(is_value)
      : v(), alloc_(&owner.allocator()) {}

  json(rapidjson::Value *value, allocator_type *alloc)
    requires(is_view)
      : v(value), alloc_(alloc) {}

  json(const json &) = delete;
  json &operator=(const json &) = delete;

  json(json &&other) noexcept : v(std::move(other.v)), alloc_(other.alloc_) {
  ...
  const char *to_string(rapidjson::StringBuffer &buffer,
                        bool is_pretty = false) const {
    if (!valid()) {
  ...
  /**
   * @brief Set an object member from another json wrapper by deep-copying its
   * value.
   *
   * This overload always performs a deep copy via RapidJSON `CopyFrom()`, even
   * when `data` refers to a temporary object. Use the rvalue overload when the
   * source json value is no longer needed and you want to avoid the extra copy
   * when allocators match.
   *
   ...
   * @return ERR_OK on success, or ERR_NOT_SUPPORT when the destination has no
   * allocator or the source json is invalid.
   */
  template <class JV> err_t set(const std::string &key, const json<JV> &data) {
  ...
  /**
   * @brief Set an object member from an rvalue json value, preferring move
   * semantics.
   *
   * When the source and destination share the same allocator, this overload
   * moves the underlying RapidJSON value into the destination member without a
   * deep copy. If the allocators differ, it falls back to a deep copy to keep
   * ownership safe.
   *
   * Prefer this overload over the const-reference overload when performance
   * matters and the source json value is no longer needed after the call.
   *
   * @param key Object member name to update or insert.
   ...
   * allocator or the source json is invalid.
   */
  template <class JV> err_t push(const json<JV> &data) {
    if (!has_allocator() || !data.valid()) {
      return ERR_NOT_SUPPORT;
    }
    rapidjson::Value copy;
   ...
   *
   * When the source and destination share the same allocator, this overload
   * moves the underlying RapidJSON value into the array without a deep copy. If
   * the allocators differ, it falls back to a deep copy to keep ownership safe.
   *
   * Prefer this overload over the const-reference overload when performance
   * matters and the source json value is no longer needed after the call.
   *
  ...
  iterator begin_array() { return begin(); }

  iterator end_array() { return end(); }

  bool is_null() const { return valid() && value().IsNull(); }

  bool is_object() const { return valid() && value().IsObject(); }

  bool is_array() const { return valid() && value().IsArray(); }

  bool is_string() const { return valid() && value().IsString(); }

  bool is_bool() const { return valid() && value().IsBool(); }

  bool is_int() const { return valid() && value().IsInt(); }

  bool is_int64() const { return valid() && value().IsInt64(); }

  bool is_double() const { return valid() && value().IsDouble(); }

  bool is_number() const { return valid() && value().IsNumber(); }

  rapidjson::Value &raw() { return value(); }

  const rapidjson::Value &raw() const { return value(); }
...
private:
  template <class> friend class json;
  ...
  void bind(rapidjson::Value &value, allocator_type &alloc)
    requires(is_view)
  {
    v = &value;
    alloc_ = &alloc;
  ...
  err_t set_value(const std::string &key, rapidjson::Value &data) {
    ...
    if (itr != value().MemberEnd()) {
      ...
      value().AddMember(rapidjson::Value(key.c_str(),
                                         (rapidjson::SizeType)key.size(),
                                         allocator())
                            .Move(),
                        data.Move(), allocator());
    }
...
template <class OwnerV> json(json<OwnerV> &) -> json<rapidjson::Value>;
