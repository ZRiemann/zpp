 ...
 * @note
 *  1. @c thr_status_ stores per-thread statistics such as state, task count, and cycle times.
 *  2. @c taskflows_ is an array of single-producer-multiple-consumer (spmc) queues that record task timestamps for each worker.
...
public:
    ...
    void set_up(size_t num_workers) override final {
        ...
        thr_status_ = static_cast<thr_stat_t*>(aligned_malloc(sizeof(thr_stat_t) * num_workers, alignof(thr_stat_t)));
        if(!thr_status_){
...
#if ENABLE_TIMESTAMPS_TRACE
        // taskflows_ init
        taskflows_ = static_cast<spmc<task_timestamps>**>(aligned_malloc(sizeof(spmc<task_timestamps>*) * num_workers, alignof(spmc<task_timestamps>*)));
...
#if ENABLE_TIMESTAMPS_TRACE
        thread_local fifo<task_timestamps>& _timestamps = taskflows_[wv.id()]->fifo_;
        if(_timestamps.not_full()){
