 ...
 * @brief Redirects spdlog output to HPX's distributed console stream
 * (hpx::cout).
 *
 * Use this sink to aggregate logs from multiple HPX localities (nodes) to the
 * root console.
 *
 * @section Warning
 * **Performance Implication**: Logging here triggers network serialization and
 * transmission to Locality 0. High-frequency logging causes network congestion.
 *
 * @section Use Case
 * **Best Practice (Mixed Strategy)**:
 * - **INFO/DEBUG**: Use local file sinks independent on each node
 * (Performance).
 ...
 *     auto logger = std::make_shared<spdlog::logger>("hpx_multi",
 * sinks.begin(), sinks.end()); logger->set_pattern("%+ [Loc:%d] [Th:%t]");
