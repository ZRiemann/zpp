...
public: // constructors & destructors
  ...
  mpmc(std::size_t chunk_size, std::size_t chunk_capacity)
      : chunk_size_(chunk_size > 0 ? chunk_size : 1),
        chunk_capacity_(chunk_capacity > 2 ? chunk_capacity : 2), chunk_num_(0),
        spare_chunk_(nullptr), front_chunk_(nullptr), back_chunk_(nullptr),
        front_ptr_(nullptr), front_end_ptr_(nullptr), back_local_(nullptr),
        back_ptr_(nullptr), back_cached_(nullptr), back_end_ptr_(nullptr) {
    ++chunk_capacity_; // for spare chunk
   ...
   */
  mpmc(std::size_t capacity) : mpmc(capacity / 2, 2) {}
  ~mpmc() {
...
public: // Produce
  ...
  inline T &back() noexcept { return *back_local_; }
...
private: // help functions
  inline chunk *alloc_chunk() noexcept {
    ...
    chk = static_cast<chunk *>(aligned_malloc(
        sizeof(chunk) + sizeof(T) * chunk_size_, MPMC_CHUNK_ALIGMENT));
    if (chk) {
      chk->next = nullptr;
