...
template <timer_sync_mode Mode = timer_sync_mode::relaxed> class timer {
public:
...
public:
  timer() noexcept : ticks_(now_ticks()) {}
  ...
  void update() noexcept { ticks_.store(now_ticks(), store_order()); }
...
private:
  template <typename Duration> std::int64_t elapsed() const noexcept {
