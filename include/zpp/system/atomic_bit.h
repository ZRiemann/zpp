 ...
 *  测试结果3线程生产3线程消费，总吞吐量 7,890,272 严重不匹配 fifo SISO 的
 * 300,000,000 吞吐量级别 考虑不使用atomic保证的非严格的同步，使用非atomic +
 * 公平队列方式实现高并发
...
public:
  ...
  void clear_bit(T state) { bits_ &= ~state; }
  void set_bit(T state) { bits_ |= state; }
  bool test_bit(T state) { return (bits_ & state) != 0; }
  /**
...
private:
  ...
  size_t find_msb(T num) {
    ...
    // GCC 或 Clang 编译器
    // if
    // constexpr是C++17引入的特性，它在编译时进行条件判断，在运行时没有任何条件判断开销
...
template <typename T> class atomic_bit {
public:
  atomic_bit() { bits_.store(0); }
  ~atomic_bit() = default;
  ...
  bool try_pop_lsb(size_t &index) noexcept {
    ...
    return bits_.compare_exchange_strong(
        bits, bits & ~(static_cast<T>(1) << index), std::memory_order_acquire,
        std::memory_order_relaxed);
  }
  /**
  ...
  bool pop_lsb(size_t &index) {
    ...
    while (bits != 0) {
      ...
      if (bits_.compare_exchange_weak(
              bits, bits & ~(static_cast<T>(1) << index),
  ...
  void notify_all() { bits_.notify_all(); }
