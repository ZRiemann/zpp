...
#if !ZPP_WALL_TIME_FORCE_LEGACY && defined(__cpp_lib_format) && __cpp_lib_format >= 201907L
...
#if !ZPP_WALL_TIME_FORCE_LEGACY && defined(__cpp_lib_chrono) && __cpp_lib_chrono >= 201907L
...
#define ZPP_WALL_TIME_USE_STD_CHRONO_FORMAT (ZPP_WALL_TIME_HAS_STD_FORMAT && ZPP_WALL_TIME_HAS_CHRONO_TIMEZONE)
#endif
...
struct wall_time {
    ...
    {
        return std::chrono::duration_cast<std::chrono::microseconds>(clock_type::now().time_since_epoch()).count();
    }
    ...
    {
        const auto microsecond_tp = std::chrono::floor<std::chrono::microseconds>(tp);
#if ZPP_WALL_TIME_USE_STD_CHRONO_FORMAT
        try {
            const auto local_time = std::chrono::zoned_time{std::chrono::current_zone(), microsecond_tp};
...
private:
    static std::string& to_str_legacy(std::chrono::time_point<clock_type, std::chrono::microseconds> tp,
                                      std::string& out)
    {
    ...
    static bool to_local_tm(std::time_t seconds_since_epoch, std::tm& out) noexcept
    {
