...
public:
  ...
  inline bool pop(T *&obj) {
    ...
    if (!_queue.try_dequeue(ctoken, obj)) {
      // spd_war("FAILED to pop obj from pool: {} cur_size[{}]", fmt::ptr(obj),
      // _queue.size_approx());
      return false;
    }
    return true;
  ...
  inline bool push(T *obj) {
    ...
    // 并不是预先分配了N的对象，pop N个对线，多线程push 回 N
    // 个对象是一定能成功的。中间用了hash算法！
    bool ret = _queue.enqueue(ptoken, obj);
    if (!ret) {
      // spd_war("try enqueue failed, delete obj{} size[{}]", fmt::ptr(obj),
      // _queue.size_approx()); delete obj; // not delete, obj monitor free
    }
    return ret;
  }
  void release() {
