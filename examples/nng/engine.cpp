...
engine::engine(int argc, char **argv)
    : nng::engine(argc, argv), _task_timer(2000) {
...
void engine::test_msg() {
  ...
  spd_inf("pub created msg: {} {} {} {}",
          std::string((char *)msg.body(), msg.length()), u16, u32, u64);
}
...
void engine::test_task_foo() {
  ...
  // total tasks:10000000 tasks_elpased:16022[avg:0]
  // idels_elapsed:565336916[avg:56]
  foo_t *foo{nullptr};
  for (int i = 0; i < task_foo::test_num; ++i) {
    if (task_foo::pool && task_foo::pool->acquire(foo)) {
      // spd_inf("dispatch foo[{}]", fmt::ptr(foo->_worker));
      if (foo->dispatch()) {
        // dispatched successfully
      } else {
        spd_err("dispatch failed");
      }
    } else {
      --i;
      continue;
