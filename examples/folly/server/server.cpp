...
app::server::server(int argc, char **argv) : fo::server(argc, argv) {
  instance = this;
}
app::server::~server() { instance = nullptr; }
...
err_t app::server::on_timer() {
  ...
  // mission_cv() 是 prvalue（临时对象）。直接写 add_cpu(mission_cv())
  // 就是传入一个右值， 模板的转发引用 Awaitable&& 会按需移动，不需要显式
  // std::move。 add_cpu(std::move(mission_cv()));
