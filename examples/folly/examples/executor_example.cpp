...
extern "C" void c_func(int x) { spd_inf("c_func({})", x); }
 ...
 * 避免把大对象按值捕获到 lambda（若捕获大对象，考虑捕获指针/引用或
 * shared_ptr）。 如果确实需要 std::bind（例如与现有 API
 * 兼容），注意闭包尺寸：可在编译期用 sizeof 检查是否会超出 folly::Function
 * 的内置缓冲区，从而触发堆分配。
 * 若极端性能敏感，避免类型擦除（即不要把可调用体包装成
 * folly::Function）；改用模板接口直接接受可调用（编译期多态，可内联调用）。 但
 * Executor 接口定义为接收
 * folly::Function，这限制了这一替代方案，除非你改变接口或实现一个模板化的
 * fast-path。
 */
void executor_example::inline_executor_example() {
  ...
  // 原来的 ka 仍然可用
  ka.copy().add([](auto &&keep) { spd_inf("keepAlive ex copy"); });
  // ka 不可用
...
void executor_example::executor_future_example() {
  folly::InlineExecutor ex;
  folly::Promise<int> p;
  // SemiFuture：不含（或不固定）执行器；适合库/API
  // 返回，允许调用者选择执行上下文（用 .via(...)）。
...
static void manual_executor_example_basic() {
  ...
  ex.add([]() { spd_inf("manual executor basic example..."); });
  ex.drive();
}
...
static void queued_immediate_executor_example_basic() {
  ...
  ex.add([]() { spd_inf("queued immediate executor basic example..."); });
}
...
static void nest_queued_immediate_executor_example() {
  ...
  x.add([&] {
    ...
    x.add([&] {
      ...
      x.add([&] { spd_inf("inline level 3: {}", ++counter); });
      spd_inf("inline level >2: {}", ++counter);
    });
    spd_inf("inline level >1: {}", ++counter);
  });
  ...
  ex.add([&ex, &counter]() {
    ...
    ex.add([&ex, &counter]() {
      ...
      ex.add([&counter]() { spd_inf("queued level: 3 {}", ++counter); });
      spd_inf("queued level >2: {}", ++counter);
    });
    spd_inf("queued level >1: {}", ++counter);
  });
...
void executor_example::benchmark_queued_immediate_executor() {
  ...
  auto tinyWork = [](size_t &counter) { counter++; };
