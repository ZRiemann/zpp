...
public:
  ...
  void
  example_pipeline_async(); // Deprecated: use example_pipeline_event_driven
  void example_pipeline_event_driven();
  // void example_pipeline_driver(); // deprecated: use
  // example_pipeline_event_driven or example_pipeline_mpsc
...
server::server(int argc, char **argv)
    : z::server(argc, argv),
      _executor(z::sys::physical_cores(),
...
void server::example_pipeline() {
  ...
  tf::Pipeline pipeline(
      ...
      tf::Pipe{tf::PipeType::SERIAL,
               [&buffer](tf::Pipeflow &pf) {
                 ...
                 spd_inf("token:{}, line:{}, pipe:{}, buf:{};", pf.token(),
                         pf.line(), pf.pipe(), buffer[pf.line()][pf.pipe()]);
               }},
  ...
  tf::Task init =
      taskflow.emplace([]() { spd_inf("ready"); }).name("starting pipeline");
  tf::Task task = taskflow.composed_of(pipeline).name("pipeline");
  tf::Task stop =
      taskflow.emplace([]() { spd_inf("stopped"); }).name("pipeline stopped");
 ...
 * @deprecated This method is obsolete due to its blocking nature. Use
 * example_pipeline_event_driven instead.
 * @note An asynchronous pipeline that fetches data from an external source.
 * @warning This implementation uses a blocking wait (cv.wait) which occupies a
 * worker thread throughout the lifecycle. Scalability Risk: If N concurrent
 * pipelines run on M workers (where N >= M), all workers will be blocked
 * waiting for data, causing Thread Pool Exhaustion and freezing the entire
 * system. Use this ONLY when you can guarantee N_pipelines < N_workers or using
 * dedicated threads. For high-concurrency systems, prefer
 * `example_pipeline_event_driven()` (Reactive Pattern).
 */
void server::example_pipeline_async() {
  ...
  // Note: In a real app, these would be members of the class or passed in
  // contexts
  ...
  tf::Pipeline pipeline(
      ...
      tf::Pipe{tf::PipeType::SERIAL,
               [&](tf::Pipeflow &pf) {
                 ...
                 queue_cv.wait(lock, [&]() {
                   return !data_queue.empty() || stop_signal;
                 });
                 ...
                 spd_inf("Pipe1 (Fetch): Token {} / Line {} got data {}",
                         pf.token(), pf.line(), val);
               }},
      ...
      tf::Pipe{tf::PipeType::PARALLEL,
               [&](tf::Pipeflow &pf) {
                 ...
                 spd_inf("Pipe2 (Process): Token {} / Line {} processed {}",
                         pf.token(), pf.line(), val);
               }},
      ...
      tf::Pipe{tf::PipeType::SERIAL, [&](tf::Pipeflow &pf) {
                 int val = line_data[pf.line()];
                 spd_inf("Pipe3 (Sink): Token {} / Line {} finished {}",
                         pf.token(), pf.line(), val);
               }});
 ...
 * @note An event-driven (reactive) pipeline that only runs when data is
 * available.
 * @attention Advantages (Reactive Pattern):
 *            1. Zero Resource Waste: Workers are released immediately when
 * queue is empty (pf.stop()).
 *            2. High Scalability: Supports M > N scenarios (M pipelines, N
 * workers) without deadlock.
 *            3. No Busy Waiting: Unlike async version, no threads are blocked
 * on cv.wait(). Recommended for high-concurrency server applications sharing a
 * global thread pool.
 */
void server::example_pipeline_event_driven() {
      ...
      tf::Pipe{tf::PipeType::SERIAL,
               [ctx](tf::Pipeflow &pf) {
                 ...
                 if (ctx->queue.empty()) {
                   ...
                   spd_inf("Pipe1 (Stop) token:{}, line:{}, pipe:{}",
                           pf.token(), pf.line(), pf.pipe());
                   pf.stop();
                 } else {
                   int val = ctx->queue.front();
                   ctx->queue.pop();
                   ctx->buffer[pf.line()] = val;
                   spd_inf("Pipe1 (Fetch): token:{}, line:{}, pipe:{}, val:{}",
                           pf.token(), pf.line(), pf.pipe(), val);
                 }
               }},
      tf::Pipe{tf::PipeType::PARALLEL,
               [ctx](tf::Pipeflow &pf) {
                 ...
                 spd_inf("Pipe2 (Process): token:{}, line:{}, pipe:{}, val:{}",
                         pf.token(), pf.line(), pf.pipe(), val);
               }},
      tf::Pipe{tf::PipeType::SERIAL, [ctx](tf::Pipeflow &pf) {
                 int val = ctx->buffer[pf.line()];
                 spd_inf("Pipe3 (Sink): token:{}, line:{}, pipe:{}, val:{}",
                         pf.token(), pf.line(), pf.pipe(), val);
               }});
  taskflow.composed_of(pipeline);

  // Reschedule Logic: A recursive-style lambda to keep the pipeline running as
  // long as data exists We use std::function to allow recursive reference
  // (captured by reference, but function object must exist) Note: We need to be
  // careful with lifetime of this function object. Since we block the main
  // thread at the end, local reference is safe.
  ...
  run_pipeline_batch = [this, ctx, &taskflow, &run_pipeline_batch]() {
    ...
    // Submit the taskflow.
    // When it finishes (because Pipe1 stopped on empty queue), we verify if we
    // need to restart.
  ...
  std::thread producer([ctx, &run_pipeline_batch]() {
    for (int i = 1; i <= 20; ++i) {
      z::sleep_ms(
          20); // Produce slower than consume to test stopping/restarting
      bool trigger_needed = false;
      {
        ...
        if (!ctx->is_running) {
          // Temporarily mark true to prevent double submission before
          // run_pipeline_batch grabs lock But run_pipeline_batch re-checks
          // logic. Let's just trigger. The lock inside run_pipeline_batch
          // handles concurrency. However, we shouldn't submit multiple
          // concurrent taskflows for the same pipeline object if it's not
          // designed for it (Pipeline holds state). So we only trigger if
          // !is_running.
          if (!ctx->is_running) {
            trigger_needed = true;
            // Optimization: Set running=true here to block other producer
            // pushes from triggering? But run_pipeline_batch sets it. To be
            // safe against race where run_pipeline_batch hasn't acquired lock
            // yet:
            ctx->is_running = true;
          }
        }
  ...
  // Wait for the pipeline to fully drain (consumer might still be working on
  // last batch)
  while (true) {
    {
      std::lock_guard<std::mutex> lock(ctx->mutex);
      if (!ctx->is_running && ctx->queue.empty())
        break;
    }
    z::sleep_ms(10);
  }
...
void server::example_pipeline_mpsc() {
  ...
  tf::Pipeline pipeline(
      ...
      driver->build_source([&buffer](tf::Pipeflow &pf, int &&val) {
        buffer[pf.line()] = val;
        spd_inf("Pipe1 (Fetch): token:{}, line:{}, pipe:{}, val:{}", pf.token(),
                pf.line(), pf.pipe(), val);
      }),
...
static void base_example(tf::Executor &executor) {
  ...
  tf::Task A = taskflow.emplace([&data]() { spd_inf("TaskA"); });

  A.name("Task_A");
  tf::Task B = taskflow.emplace([&data]() { spd_inf("TaskB"); });
  B.name("Task_B");
  tf::Task C = taskflow.emplace([&data]() { spd_inf("TaskC"); });
  C.name("Task_C");
  tf::Task D = taskflow.emplace([&data]() { spd_inf("TaskD"); });
  D.name("Task_D");
  ...
  executor.run(taskflow).wait();
  spd_inf("After adding subflow:{}; num_tasks:{}", taskflow.dump(),
          taskflow.num_tasks());
  executor.run(taskflow).wait();
  /*
  ...
  */
  spd_inf("Run again subflow:{}; num_tasks:{}", taskflow.dump(),
          taskflow.num_tasks());

  taskflow.for_each_task(
      [](tf::Task t) { spd_inf("Visiting task: {}", t.name()); });
...
static void data_driven_example(tf::Executor &executor) {
  std::vector<decltype(executor.run(std::declval<tf::Taskflow &>()))>
      futs; // hold futures
  std::vector<std::shared_ptr<tf::Taskflow>>
      kept_taskflows; // keep taskflow objects alive
  for (int run_id = 0; run_id < 3; ++run_id) {
    ...
    taskflow->emplace(
        [data, run_id]() { spd_inf("Run {} - TaskB (uses data)", run_id); });
  ...
  // wait for all runs to complete (ensures all `data` are still alive until
  // done)
  for (auto &f : futs) {
    f.wait();
  }
...
static void task_example(tf::Executor &executor) {
  ...
  For example, the code below creates a static task and then reworks it to a
  subflow task:
  */
  task1
      .work([](tf::Subflow &sf) {
  ...
  // dependencies
  {
    ...
    auto [init, cond, yes, no] = taskflow.emplace(
        ...
        []() { std::cout << "no\n"; });
    cond.succeed(init).precede(yes, no); // executes yes if cond returns 0
                                         // executes no  if cond returns 1
    spd_inf("Conditional tasking:\n{} num_strong_dependencies:{}, "
            "num_weak_dependencies:{}",
            taskflow.dump(), cond.num_strong_dependencies(),
            cond.num_weak_dependencies());
  }
  ...
  // class test
  {
    ...
    public:
      ...
      A(int v) : _value(v) {}
      void print() { spd_inf("Value is {}", _value); }
  ...
  // reset task
  {
    ...
    executor.run(taskflow).wait();
    // task.reset(); // reset() 只能让 task 变成 empty task，但是taskflow
    // 中的节点还在，会被执行；
    task.reset_work(); // reset_work() 会把任务变成 placeholder，不会被执行。but
                       // not empty()
  ...
  // applies an visitor callable to each successor of the task
  {
    ...
    A.for_each_successor(
        [](tf::Task t) { spd_inf("Successor task: {}", t.name()); });

    D.for_each_predecessor(
        [](tf::Task pred) { spd_inf("Predcessor task:{};", pred.name()); });
    // executor.run(taskflow).wait();

    tf::Taskflow tf_1;
    tf::Task task =
        tf_1.emplace([](tf::Subflow &sf) {
              tf::Task stask1 =
                  sf.emplace([]() { spd_inf("stask1"); }).name("stask1");
              tf::Task stask2 =
                  sf.emplace([]() { spd_inf("stask2"); }).name("stask2");
              (void)stask1;
              (void)stask2;
    ...
    /*
    要想访问子任务，1）必须在子任务函数中调用 sf.retain(true);
    这样子任务才会被保留， 2）然后在运行 taskflow 之后，才能访问子任务
    */
    task.for_each_subflow_task(
        [](tf::Task stask) { spd_inf("subflow task {}", stask.name()); });
  }
}
...
void example_runtime(tf::Executor &executor) {
  ...
  std::shared_ptr<tf::ChromeObserver> observer =
      executor.make_observer<tf::ChromeObserver>();
  executor.run(taskflow).wait();
